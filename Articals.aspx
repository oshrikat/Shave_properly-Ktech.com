<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Articals.aspx.cs" Inherits="Giluah_Kehalaha___NEW_WEBSITE_2022.Articals" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml" dir="rtl">
<head runat="server">
    <title>מאמרים</title>
    <link href="articals.css" rel="stylesheet" />
        <link rel="icon" type="image/x-icon" href="https://irp-cdn.multiscreensite.com/e4ac1c0a/site_favicon_16_1566894260487.ico"/>
       <link rel="stylesheet"  href="HomePage.css" />
   <link rel="stylesheet"  href="HomePage.css" />

<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css"/>
 
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css"/>
 
</head>
<body><section class="header">
        <nav>
            <div class="div1">
                

                <ul id="menu">

                    <a href="Home_Page.aspx" class="nevigation" >
                    <li>    דף הבית   </li>
                    </a>

                    <a href="Odot.aspx" class="nevigation">
                    <li>    אודות  </li>
                    </a>

                    <a href="Pictures.aspx" class="nevigation">
                    <li>    תמונות   </li>
                    </a>

                    <a href="Articals.aspx" class="nevigation" style="border-bottom:1px double white">
                    <li>  מאמרים     </li>
                    </a>

                    <a href="Contact Us.aspx" class="nevigation">
                    <li>     צור קשר </li>
                    </a>

                    <li>
                    <a href="Home_Page.aspx" id="nevigation">
                        <img src="data:image/png;base64," width="300" height="43" id="shaver" title="גילוח כהלכה"/>
                    </a>
                    </li>

                    
                   
                  <li class="Acount" style="font-family: Impact, Haettenschweiler, 'Arial Narrow Bold', sans-serif; margin-top:0.5% ;"> 
                   &nbsp;  <%=Acount %>   </li>
                       
                </ul>

            </div>


        </nav>        







    </section>
    <br />
    <br />
    <br />
    <br />
    <br />
    
    
    <section >
        <div id="title">
            <h1 id="title_mahamarim"> מאמרים  </h1>
        </div>
    </section>

    <div id="tmonna">
       <%--
        <img src="pictures/מכונת קייטק2.png" id="tmonna2" />
        <img src="pictures/קייטק.jpg" id="tmonna3" />
        <img src="pictures/תספורת קייטק.png"  id="tmonna4"/>--%>
    </div>
    

    <article class="text">
        <div id="background">
              <img src="pictures/מכונת קייטק2.png" id="tmonna1"/>
        <div id="firstP">
        <b>עשינו היסטוריה -  </b> מכונת גילוח כשרה למהדרין
        בתום 9 שנים של הליך ארוך וממושך בהשגחת בית הדין
        <br>
        שהוקם במיוחד לנושא

        "בית הדין המיוחד לענייני מכונות גילוח כהלכה"  בראשות כב' הגאון הרב מרדכי
        <br/> גרוס שליט"א
        ובליווי צוות של מחקר ופיתוח הכולל:
        <br/>
        <br />


        תלמידי חכמים, הרב הגאון אליהו דרעי זצ"ל מהנדס ומומחה למכונות גילוח שהודרכו<br /> בהוראותיו
        המפורשות והישירות של :<br /> מרן הגרי"ש אלישיב זצ"ל , ולמדו ביסודיות ובדקדקנות את מורכבות
        נושא הגילוח,
        נערכו מאות בדיקות קפדניות על מגוון סוגי עור וזקן הן של מבוגרים והן של צעירים תוך הפעלת לחץ בכל אזורי הפנים ובפרט באזורים הרגישים לגילוח ורק לאחר שנמצא כי המגע של ראשי הגילוח בעור הפנים מאפשר גילוח נאה ללא חשש השחתה בכפוף לסייגים הלכתיים
        בהתאם להוראותיהם של גדולי הפוסקים ובפרט לפי  הוראותיו <br>
        המפורשות והישירות של:
        <br />
        פוסק הדור מרן הגרי"ש אלישיב שליט"
        יוצרה ואושרה מכונת גילוח ייחודית " כשרה מהדרין" שעברה שינויים ושיפורים המאפשרים גילוח ללא חשש השחתה של חב' קייטק.
        ראשי הגילוח של המכונה נבדקים בקפידה באמצעות מכשירי מדידה מדויקים אחד אחד
        על כל מכונה או ראשי גילוח קיימת מדבקה ממוספרת המאשרת שהמכונה בדוקה ע"י
        "בית הדין המיוחד לענייני מכונות גילוח כהלכה"
        וכך מתאפשר גילוח כשר מהדרין ע"פ ההלכה לכל סוגי הזקנים .
            </div>
               
            </div>
 
            <div id="background2">
                  <img src="pictures/תספורת קייטק.png" id="tmonna2" />
        <div id="secondP">
          
       <b> הכול מתחיל לפני כ 72 שנים</b> קל רבי צבי פסח פרנק  ואישר שימוש במכונת גילוח חשמלית מהדגמים הראשונים שבהן הרשת עבה וסכין סטנדרטית.
        במשך שנים (עד שנת תשס"א) ההכשר שניתן למכונות גילוח נסמך על מאמר שהתפרסם  ב'תחומין' י"ג (עמ' 200)  מאת הרב שבתי רפפורט, שהציע לבדוק האם "הלהב" (הסכין) שמסתובבת מתחת לרשת של מכונות הגילוח מסוגלת לחתוך שערות  גם ללא הרשת  ומחכה פעולה של מספרים, ללא בדיקה מעמיקה בכלים וציוד מתקדם  נצפה ונקבע כי הסכין במכונות הגילוח ללא הרשת אינה גוזזת את השערה ללא הרשת ומתוך הבנה כי מכונות הגילוח מחכות פעולה של מספרים באותם שנים אישרה צומת כשרות על כל מכונות הגילוח,
        אך... מאז היו פקפוקים ביחס לשיטת הבדיקה ולאמינותה והוחלט לערוך בדיקה מדוקדקת
        בתצלומי תקריב שתוצאתם ערערה הנחה זו,
        המסקנות פורסמו ב'תחומין' כ"ב אייר תשס"ב
        מסקנת הבדיקה: לא ניתן להגדיר הבדל משמעותי מבחינת התוצאה ומבחינה הלכתית  - בין הדגמים השונים אם ה ''תוצאה'' היא הקובעת אזי כל המכונות אסורות שהרי כולן לא משאירות שערות כדי אחיזה בהן או אפילו כדי יכולת מישוש.
        השאלה האם מכונות הגילוח המשופרות בנות זמננו עדין מותרות - הובילה ליוזמה שהחלה בשנת השמיטה האחרונה בעקבות הוראת מרנן ורבנן שליט"א וזצוק"ל בקול קורא לבני הישיבות שלא להשתמש במכונות הגילוח המצויות בשוק בשל חשש ממשי שכולן משחיתות כתער ולקריאה זו הצטרפו ראשי ישיבות.

        <br>  <br>
        מאוחר יותר בשנת תשע"ב יצאו רבנים במספר ערים בתקנה מיוחדת "לאפרושי מאיסורא"
        ובה שבעה סעיפים בדיני גילוח ותספורת, שניים מתוכם מתייחסים לנושא מכונות הגילוח וקובעים שכל סוגי מכונות הגילוח אסורות ודינם כגילוח בתער ולכן נאסר למוכרן במספר ערים, כנראה בשל העובדה שלא ניתן היה להטיל גזירה שהציבור לא יכול לעמוד בה - כי באותה עת לא הייתה מכונה כשרה,...
        נמכרו מכונות למרות אזהרת בית הדין !! <br>  <br>
            </div>


                </div>
          <div id="background3">
              <img src="pictures/תספורת שלב 3.jpg" id="tmonna3" />
        <div id="thirdP">
        כל החברות המובילות שמייצרות מכונות גילוח מדגישות ומפרסמות את השיפורים הרבים במבנה ודקיקות הרשת שמקנים תוצאת " גילוח חלק יותר ועמוק יותר "
        .במהלך הבדיקות בצילומי תקריב באמצעות מצלמה מקרוסקופית בין השאר נמצא ע"י צוות המומחים מטעם בית הדין כי השיפורים הרבים של רשתות וסכיני המכונות גילוח בנות זמננו של כל החברות קרובות כחוט השערה לגילוח בתער ומשחיתות, בזמן הגילוח מופעל לחץ הגורם להדחקות של עור הפנים אל תוך הרשת פנימה נמצא כי קיים מגע מלטף נוגע/ לא נוגע של הסכין  באזורים מסוימים בעור הפנים בכל המכונות המשופרות החדשות גם כאשר בוצע שבירה והסרת הקפיץ מהסכין במטרה להכשיר את הסכינים.
        <br/>  <br/>
        ולצערנו ישנם רבים שנכשלו בכך מחוסר ידע.
        במהלך השנים נעשו ניסיונות לפנות לחברות המובילות ביניהם פיליפס
        ,   בראון ועוד... שכשלו
        מאחר וכל החברות חשבו כי מדובר בפרויקט ארוך מתיש יקר עם פוטנציאל קטן שלא מסחרי וכלכלי עבורם.
        <br>  <br>
        מזה ארבע שנים,
        משיש מכונת גילוח שעברה שינויים ייחודיים לשם גילוח ללא השחתה של חב' קייטק
        דגם KS-6 ופאתר מדגם KT-7 של חברת K-tech שהינן היחידות שכשרות מהדרין,
        <br> <br>
        חלה התעוררות רבה גם אצל מתחזקים, מסורתיים, בחורי ישיבות, ריכוזי יהודים ברחבי העולם וכלל הזרמים שברכו את היזם על פתרון הבעיה שהטרידה את גדולי הדור מזה מס' שנים.

        <br>
        היזם שנבחר ע"י הרב גרוס ואושר ע"י כב' הרב אלישיב זצ"ל השקיע את מיטב כספו וזמנו במשך 9 שנים קיבל אישור מ "בית הדין המיוחד לענייני מכונות גילוח כהלכה "
        ורשם את הפיתוח של המכונה היחודית כפטנט רשום במשרד הפטנטים.

            </div>

              </div>



    </article>
    <section>
    </section>






    <hr id="hr1" />

     <section class="botton">
        <div id="auto">
    <div id="underpage">
        <div id="contact_us">
            <h3>  <u>פרטי התקשרות: </u>   </h3> 
            <br />
            <h4>  ט'ל : 03-5440445  </h4>

            <br />
            <h4> מייל : ketch.israel@gmail.com</h4>
            <br />
            <h4>  כתובת : אברבנל 11 חולון </h4>
        </div>


        <div id="pages">
            <h3>  <u> קישורים: </u>   </h3>

            <br />

            <h4> <a href="Home_Page.aspx" title="דף הבית">      דף הבית   </a>  </h4>

            <br />

            <h4>  <a href="Odot.aspx" title="אודות">        אודות  </a>   </h4>

            <br />

            <h4>   <a href="Pictures.aspx" title="תמונות">    תמונות     </a>  </h4>


            <h4 style="margin-right:10%; margin-top:-9.5%">   <a href="Articals.aspx" title="מאמרים">      מאמרים </a>    </h4>
            <br />

            <h4 style="margin-right:10%; ">   <a href="Contact Us.aspx" title="צרו קשר">      צרו קשר   </a>  </h4>
        </div>

        <div id="social">
          <a href="https://www.facebook.com/pg/K-Tech-israel-%D7%A7%D7%99%D7%99-%D7%98%D7%A7-%D7%99%D7%A9%D7%A8%D7%90%D7%9C-176094652445545/posts/" class="fa fa-facebook"    ></a>
            <a href="https://www.youtube.com/channel/UC10m3UA0H8B6N9VgyFW4V5w" class="fa fa-youtube"></a>
            <a href="https://www.instagram.com/ktech.israel/" class="fa fa-instagram"></a>

    

        </div>
     
        <a href="https://www.i-ktech.com/"> <img src="pictures/קייטק לוגו תמונה.png" id="ktech" /> </a>  
    

        <h4 id="ALLRIGHTS"> כל הזכויות שמורות לאושרי קטריבס וקייטק ישראל &copy;</h4>
            </div>
            </div>
    </section>



</body>
</html>
