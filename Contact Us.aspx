<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Contact Us.aspx.cs" Inherits="Giluah_Kehalaha___NEW_WEBSITE_2022.Contact_Us" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml" dir="rtl">
<head runat="server">
    <title>צור קשר</title>
    <link href="contact.css" rel="stylesheet" />
    <script src="https://kit.fontawesome.com/a076d05399.js" crossorigin="anonymous"></script>
    <link href="https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet"/>
        <link rel="icon" type="image/x-icon" href="https://irp-cdn.multiscreensite.com/e4ac1c0a/site_favicon_16_1566894260487.ico"/>

    <script src="contactJava.js"></script>
</head>
<body>
   
    <section class="header">
        <nav>
            <div class="div1">
                

                <ul id="menu">

                    <a href="Home_Page.aspx" class="nevigation" >
                    <li>    דף הבית   </li>
                    </a>

                    <a href="Odot.aspx" class="nevigation">
                    <li>    אודות  </li>
                    </a>

                    <a href="Pictures.aspx" class="nevigation">
                    <li>    תמונות   </li>
                    </a>

                    <a href="Articals.aspx" class="nevigation">
                    <li>  מאמרים     </li>
                    </a>

                    <a href="Contact Us.aspx" class="nevigation" style="border-bottom:1px double white">
                    <li>     צור קשר </li>
                    </a>

                    <li>
                    <a href="Home_Page.aspx" id="nevigation">
                        <img src="data:image/png;base64," width="300" height="43" id="shaver" title="גילוח כהלכה">
                    </a>
                    </li>

                   
                    
                   
                  <li class="Acount" style="font-family: Impact, Haettenschweiler, 'Arial Narrow Bold', sans-serif; margin-top:0.5% ;"> 
                   &nbsp;  <%=Acount %>   </li>
                       
                </ul>

            </div>


        </nav>        







    </section>

  
      <div id="blur_picture">
    <form name="myform" id="myform" method="post" action="AfterContact.aspx" onsubmit="return validation()">

        <div class="form">
            <div id="title">
                <h1 id="title_h1"> צור קשר  </h1>
            </div>
            <p id="error" style=" color:white;  display:flex; margin: -7% auto; width:20%; justify-content:center; "> </p>
            <div id="inputs">
                <div id="firstgroup">
                <input type="text"    onclick="clearAll()"  name="one" id="firstname" placeholder=" שם פרטי"  autocomplete="off"  /> 
                <span id="error1"></span>
                
                <input type="text"  onclick="clearAll()" name="two" id="lastname" placeholder=" שם משפחה" autocomplete="off" /> 
                <p id="error2"></p>
                    </div>
                                <div id="secondgroup">

                <input type="text"  onclick="clearAll()" name="three" id="email" placeholder=" כתובת אימייל"autocomplete="off" />
                <p id="error3"></p>
                <input type="number"  onclick="clearAll()" name="four" id="number" placeholder="הזן מספר טלפון" autocomplete="off" />
                <p id="error4"></p>

              </div>
                <div id="message">
                    <textarea id="textarea" name="comment" placeholder="יש שאלות? רוצים לקבל עזרה בפרטני? כל דבר תכתבו לנו כאן!"></textarea>
                <input type="submit" id="submit"/>
                    </div>

                <br />
                <br />


             


                 

            </div>
             

        </div>
    </form>
          <section id="iconsCharge">
            
          <i class="material-icons" style="font-size:40px; padding-top:1%; padding-right:1%">call</i>
          <h4 id="phoneNumber">טלפון: 03-5440445</h4>
      
              </section>

          <section  id="iconsCharge2">
<i class='fas fa-at' style='font-size:40px; color:black; padding-top:1%;padding-right:1% '></i>
                 <h4 id="Email">מייל: ktech.israel@gmail.com</h4>
        </section> 
          <section  id="iconsCharge3">
<i class='fas fa-map-marker-alt' style='font-size:40px; padding-top:1%;color:black;padding-right:2%'></i>
                 <h4 id="location">כתובת: אברבנאל 11, חולון</h4>
        </section>
          </div>
</body>
</html>