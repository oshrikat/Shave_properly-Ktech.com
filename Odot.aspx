<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Odot.aspx.cs" Inherits="Giluah_Kehalaha___NEW_WEBSITE_2022.Odot" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml" dir="rtl">
<head runat="server">
    <title> אודות</title>
    <link href="odot.css" rel="stylesheet" />
        <link rel="icon" type="image/x-icon" href="https://irp-cdn.multiscreensite.com/e4ac1c0a/site_favicon_16_1566894260487.ico"/>
       <link rel="stylesheet"  href="HomePage.css" />
    <link rel="icon" type="image/x-icon" href="https://irp-cdn.multiscreensite.com/e4ac1c0a/site_favicon_16_1566894260487.ico"/>

<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css"/>
 
</head>
<body>
     <section class="header">
        <nav>
            <div class="div1">
                

                <ul id="menu">

                    <a href="Home_Page.aspx" class="nevigation" >
                    <li>    דף הבית   </li>
                    </a>

                    <a href="Odot.aspx" class="nevigation" style="border-bottom:1px double white">
                    <li>    אודות  </li>
                    </a>

                    <a href="Pictures.aspx" class="nevigation">
                    <li>    תמונות   </li>
                    </a>

                    <a href="Articals.aspx" class="nevigation">
                    <li>  מאמרים     </li>
                    </a>

                    <a href="Contact Us.aspx" class="nevigation">
                    <li>     צור קשר </li>
                    </a>

                    <li>
                    <a href="Home_Page.aspx" id="nevigation">
                        <img src="data:image/png;base64," width="300" height="43" id="shaver" title="גילוח כהלכה"/>
                    </a>
                    </li>

                    
                    
                   
                  <li class="Acount" style="font-family: Impact, Haettenschweiler, 'Arial Narrow Bold', sans-serif; margin-top:0.5% ;"> 
                   &nbsp;  <%=Acount %>   </li>
                       
                </ul>

            </div>


        </nav>        







    </section>

    <hr id="hr1" />
    <div id="title">
        <h1 id="title_odot"> אודות גילוח כהלכה </h1>
    </div>

    <section class="odot">

        <div id="tmonna">
            <img src="pictures/תספורת%20קייטק.png" id="tmonna1" />
        </div>





        <div class="text">

            כשהומצאה מכונות  גילוח חשמלית ראשונה נוצר בין פוסקי ההלכה דיון
            על היתר השימוש בה לגילוח הלחיים ופאת הזקן
            עיקר הדיון התמקד על ההבחנה בין תער - האסור בשימוש, לבין מספריים - המותרים.<br />
            מלכתחילה רבים מן הפוסקים נטו להחשיב את מכונת הגילוח כתער בשל התוצאה לאחר הגילוח - השערה נחתכת סמוך מאד לעור. <br /> <br />
            לדעת גדולי הפוסקים המכונות בנות זמננו כוללות ראשי גילוח משופרים סכין ליפט אנד קאט ורשת עם חריצים, חורים ואלמנטים המשפרים את פעולת הגילוח
            ומקנים תוצאת גילוח קרובה מאוד לעור הפנים בדומה לתער.
            <br /> <br />
            השחתת הזקן של גברים הינו איסור מן התורה,
            חמישה לאווים מדאורייתא !!
            מובהר בספר ויקרא, פרשת קדושים  "אֶל כָּל עֲדַת בְּנֵי יִשְׂרָאֵל"
            לֹא תַקִּפוּ פְּאַת רֹאשְׁכֶם, וְלֹא תַשְׁחִית אֵת פְּאַת זְקָנֶךָ
            בנוסף בספר ויקרא כא, פסוק ה' מופנה אל הכהנים:
            <br /> <br />
            "וּפְאַת זְקָנָם לֹא יְגַלֵּחוּ"
            יש הרבה דעות בראשונים איפה בדיוק נמצאים חמשת הנקודות האסורות,
            לכן פסק שו"ע ביורה דעה ס´ קפ"א סע´ יא:
            "פאות הזקן הם ה´ ורבו בהם הדעות ,
            לפיכך ירא שמים יצא את כולם ולא יעביר תער על כל זקנו כלל.
            <br /> <br />
            הרמ"א מוסיף שאפילו מתחת לגרון יש איסור גילוח כיוון שהעור זז
            ולפעמים יתכן שמגלחים חלק הנמצא מעל הגרגרת ומדובר באיסור דאורייתא
            לכן יש להחמיר,
            שו"ע אסר בכל המקומות שגדל זקן  ולא נמצא אף מקל כנגד שו"ע,
            ברור לכל הפוסקים כי קשה מאד להקל באיסור דאורייתא שיש בו כ"כ הרבה ספקות.



        </div>


    </section>



       <section class="botton">
        <div id="auto">
    <div id="underpage">
        <div id="contact_us">
            <h3>  <u>פרטי התקשרות: </u>   </h3> 
            <br />
            <h4>  ט'ל : 03-5440445  </h4>

            <br />
            <h4> מייל : ketch.israel@gmail.com</h4>
            <br />
            <h4>  כתובת : אברבנל 11 חולון </h4>
        </div>


        <div id="pages">
            <h3>  <u> קישורים: </u>   </h3>

            <br />

            <h4> <a href="Home_Page.aspx" title="דף הבית">      דף הבית   </a>  </h4>

            <br />

            <h4>  <a href="Odot.aspx" title="אודות">        אודות  </a>   </h4>

            <br />

            <h4>   <a href="Pictures.aspx" title="תמונות">    תמונות     </a>  </h4>


            <h4 style="margin-right:10%; margin-top:-9.5%">   <a href="Articals.aspx" title="מאמרים">      מאמרים </a>    </h4>
            <br />

            <h4 style="margin-right:10%; ">   <a href="Contact Us.aspx" title="צרו קשר">      צרו קשר   </a>  </h4>
        </div>

        <div id="social">
          <a href="https://www.facebook.com/pg/K-Tech-israel-%D7%A7%D7%99%D7%99-%D7%98%D7%A7-%D7%99%D7%A9%D7%A8%D7%90%D7%9C-176094652445545/posts/" class="fa fa-facebook"    ></a>
            <a href="https://www.youtube.com/channel/UC10m3UA0H8B6N9VgyFW4V5w" class="fa fa-youtube"></a>
            <a href="https://www.instagram.com/ktech.israel/" class="fa fa-instagram"></a>

    

        </div>
     
        <a href="https://www.i-ktech.com/"> <img src="pictures/קייטק לוגו תמונה.png" id="ktech" /> </a>  
    

        <h4 id="ALLRIGHTS"> כל הזכויות שמורות לאושרי קטריבס וקייטק ישראל &copy;</h4>
            </div>
            </div>
    </section>


</body>
</html>